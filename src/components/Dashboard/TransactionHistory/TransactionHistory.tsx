...
export default function TransactionsTable() {
    ...
    return (
        <div className="w-full bg-[#1a1a1a] rounded-xl shadow-sm py-6 px-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="px-4 py-2 pr-6 flex items-center border border-[#989898] text-white rounded-md gap-2"
                    >
            ...
            {/* Loading */}
            {isLoading && (
                <p className="text-center py-10 text-gray-300 text-md">Loading withdrawals...</p>
            )}

            {/* No data */}
            {!isLoading && filteredData.length === 0 && (
                <p className="text-center py-10 text-gray-400 text-md">No withdrawals found</p>
            )}
            ...
            {!isLoading && filteredData.length > 0 && (
                <>
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="text-gray-200 border-b border-[#ffffff50] text-[17px]">
                                ...
                                >
                                    <td className="py-4 flex items-center gap-3 text-white">
                                        ...
                                        <span className="font-medium">
                                            {([item?.user?.firstName, item?.user?.lastName].filter(Boolean).join(" ")).slice(0, 18)}
                                    ...
                                    <td className="py-4 text-center">
                                        <span
                                            className={`px-3 py-1 rounded-full border text-[12.5px] capitalize ${item.status === "pending"
                    ...
                    ${page === i + 1 ? "border-green-500 text-green-500" : ""}`}
                                >
                                    {i + 1}
                                </button>
                            ))}
